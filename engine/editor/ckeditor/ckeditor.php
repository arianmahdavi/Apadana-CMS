...
class CKEditor
{
	...
	public function editor($name, $value = "", $config = array(), $events = array())
	{
		...
		if (!empty($_config))
			$js .= "CKEDITOR.replace('".$this->textareaAttributes['id']."', ".$this->jsEncode($_config).");";
		else
			$js .= "CKEDITOR.replace('".$this->textareaAttributes['id']."');";
	...
	private function init()
	{
		...
		if (isset($this->bbcode_set)) $out .= $this->script("var bbcode_set=". $this->bbcode_set .";");
	 ...
	 * @return string
	 */
	private function jsEncode($val,$an="'")
	{
		if (is_null($val)) {
			return 'null';
		}
